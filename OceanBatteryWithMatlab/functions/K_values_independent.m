...
if phase == 1 %Indicates the charging phase.
    K_values_charging_parameters; %Loads the script K_values_charging_parameters.
elseif phase == 2 %Indicates the dicharging phase.
...
K_contraction_bend = ((1/Cc)-1)^2; %[-] Minor loss coefficient of the contraction.  

K_enlargement_bend = (1 - (A_bend/A_bend_connection))^2; %[-] Minor loss coefficient of the enlargement.
...
K_ball_valve = K_ball_valve_table(index_K_ball_valve); %[-] Minor loss coefficient ball valve. 
A_ball_valve = pi*(0.5*D_ball_valve)^2; %[m^2] Area of the ball valve. 

%% Unions       
if Union_type == 1 %indicates a threaded union.
    K_unions = 0.08; %[-] Minor loss coefficient threaded union.
elseif Union_type == 2 %indicates a flanged union.
    K_unions = 0.04; %[-] Minor loss coefficient flanged union. 
end
A_union = pi*(0.5*D_union)^2; %[m^2] Area union.

%% pressure sensor
K_pressure_sensor = 0.08; %[-] K-value of a pressure sensor.
...
if Flow_meter_type == 1 %indicates an electromagnetic flow meter.
    K_flow_meter = 0; %[-] Minor loss coefficient electromagnetic flow meter.
elseif Flow_meter_type == 2 %indicates an ultrasonic flow meter.
    K_flow_meter = 0; %[-] Minor loss coefficient ultrasonic flow meter.
elseif Flow_meter_type == 3 %indicates a turbine flow meter.
    K_flow_meter = 0.8; %[-] Minor loss coefficient turbine flow meter.
elseif Flow_meter_type == 4 %indicates a paddle wheel flow meter.
    K_flow_meter = 1; %[-] Minor loss coefficient paddle wheel flow meter.
end
A_flow_meter = pi*(0.5*D_flow_meter)^2; %[m^2] Minor loss coefficient flow meter.

%% Check valve
K_check_valve = 2; %[-] K-value for an open check valve, if the valve is closed, the K-value equals infinity.
...
elseif Inlet_type == 2 %Indicates a rounded inlet.
    ...
    K_inlet = K_inlet_rounded_table(index_K_inlet_rounded); %[-] Minor loss coefficient inlet. 
...
%% Outlets
K_outlet = 1; %[-] K-value for all outlets.
...
K_outlet_bypass = K_outlet; %[-] Minor loss coefficient outlet bypass. Is the same for every kind of outlet.

if Inlet_type_bypass == 1 %Indicates a square inlet.
    K_inlet_bypass = 0.5; %[-]K-value for an inlet with square edges.
elseif Inlet_type_bypass == 2 %Indicates a rounded inlet.
    ...
    [~,index_K_inlet_rounded] = (min(abs(rd_table-rd_ratio))); %Finds the index number of the ratio from the table that is closest to rd_ratio. 
    K_inlet_rounded_table = [0.28 0.24 0.15 0.09 0.04]; %Corresponding minor loss coefficients.
    K_inlet_bypass = K_inlet_rounded_table(index_K_inlet_rounded); %[-] Minor loss coefficient inlet. 
elseif Inlet_type_bypass == 3 %Indicates an inlet with an extended edge.
    K_inlet_bypass = 0.78; %[-]K-value for an inlet with an extended edge.
elseif Inlet_type_bypass == 4 %Indicates an inlet with a chamfered edge.
    K_inlet_bypass = 0.25; %[-]K-value for an inlet with a chamfered edge. 
end

K_bypass = K_inlet_bypass + K_outlet_bypass; %[-] Minor loss coefficient bypass.
